...
if st.session_state.stage == "zero":
    ...
    with st.status("Loading AI models & generating insights...", expanded=True) as status:
        try:
            ...
            # ✅ FIX: Call static method directly without 'self'
            gh_sum = AnalystAgent.summarize_pain_points("GitHub", d["issues"])
            hn_sum = AnalystAgent.summarize_pain_points("Social", hn) if hn else "No recent discussions"
